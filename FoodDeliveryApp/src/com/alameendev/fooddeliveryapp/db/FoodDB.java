...
public class FoodDB {
	...
	public void createUser(String userName, String password, String address, String phoneNo, char role) {
		...
		User user = new User.Builder().userId(uniqueId.getUserId()).name(userName).address(address).password(password)
				.phoneNo(phoneNo).role(type).build();
	...
	public void addHotel(String name, char type, String phoneNo, String address) {
		...
		Hotel hotel = new Hotel.Builder().phoneNo(phoneNo).hotelId(uniqueId.getHotelId()).address(address).name(name)
				.dishType(dish).adminId(getCurrentUser().getUserId()).build();
	...
	public void createDish(long hotelId, String name, int price, char dishType) {
		...
		Dish dishObj = new Dish.Builder().dishId(uniqueId.getDishId()).hotelId(hotelId).name(name).name(name)
				.userId(currentUser.getUserId()).price(price).dishType(dish).build();
	...
	public boolean addDishToCart(long dishId) {
		...
		if (dish != null) {
			Cart cart = cartList.stream().filter(c -> c.getUserId() == currentUser.getUserId()).findFirst()
					.orElse(null);
			if (cart == null) {
	...
	public Hotel getHotel(long hotelId) {
		return hotelList.stream().filter(hotel->hotel.getHotelId() == hotelId).findFirst().orElse(null);
	}
}
