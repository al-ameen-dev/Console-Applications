package com.alameendev.interviewpanel.serializer;
...
public class JsonSerializer {
	...
	public <T> T retrieveFromJsonSingleObj(String fileName, Class<T> classFile) {
		try {
			String path = System.getProperty("user.dir");
			File file = new File(path + "/Data/" + fileName);
			if (file.exists()) {
	...
	public <T> List<T> retrieveFromJson(String fileName, Class<T> classFile) {
		try {
			String path = System.getProperty("user.dir");
			File file = new File(path + "/Data/" + fileName);
			if (file.exists()) {
				...
				String json = new String(bytes);
				fileInputStream.close();
				return (List<T>) mapper.readValue(json, classFile);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
